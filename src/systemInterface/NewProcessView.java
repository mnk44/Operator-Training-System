...
public class NewProcessView extends JDialog {
	...
	private JRadioButton consecutive;
	...
	public NewProcessView(int u) throws SQLException {
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {				
				if(processName.getText().replace(" ", "").isEmpty() || anmRoute.isEmpty() || drlRoute.isEmpty()){
					...
					ProcessConfiguration config = new ProcessConfiguration(processId, (int)cantEnt.getValue(), (int)cantAprov.getValue(), 
					...
					try {
						LoadFiles.getAnmInfo(Convert.toBytes(new File(anmRoute)), processId);
					...
					try {
						LoadFiles.getDrlInfo(Convert.toBytes(new File(drlRoute)), processId);
					} catch (ClassNotFoundException | IOException
							| SQLException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
		...
		consecutive = new JRadioButton("Los intentos deben aprobarse consecutivamente:     ");
		consecutive.setHorizontalTextPosition(SwingConstants.LEFT);
		consecutive.setHorizontalAlignment(SwingConstants.LEFT);
		consecutive.setForeground(new Color(47, 46, 65));
		consecutive.setFont(new Font("Segoe UI", Font.BOLD, 22));
		consecutive.setBackground(Color.WHITE);
		consecutive.setBounds(41, 107, 625, 29);
		ent.add(consecutive);
	...
	public ArrayList<User> fillUser() throws SQLException{
	...
	public ArrayList<String> fillUserName() throws SQLException{
